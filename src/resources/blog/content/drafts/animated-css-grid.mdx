Learn how to create a dynamic, responsive grid in **Next.js** with **GSAP** animations and **Tailwind CSS**. We'll cover mapping over data arrays, dynamic styling, and scroll-triggered animations to build a reusable, mobile-friendly component that enhances your site's interactivity.

Let’s get started!

## Project Setup

Before we get started, make sure your Next.js project is ready to go. If you're starting from scratch, you can set up a new project by running:

```bash
npx create-next-app my-animated-grid
```
...
{/* -FEEDBACK
-  Could do with a basic setup section and conclusion
- IDS were being mapped causing duplicated IDs - they need to be unique - better to use className instead e.g ‘card’
- As we are using the container as useGSAP scope we can use lazy selectors like img, h5, p which is a bit simpler
- Need to register ScrollTrigger as well as useGSAP */}
...
To save you time, you can grab all the icons we’ll be using right <a href="/assets/grid-icons.zip" download="grid-icons.zip" className="underline underline-offset-2">here</a>.

{/* // MATT: I've added a type and marked the object as 'const' */}
{/* // Renamed 'title' to 'heading' and 'subtitle' to 'description' - you normally only have 1 title per page. */}
...
## 4. Animating Cards with GSAP

In this section we'll bring it to life with some smooth animations using GSAP!

We'll be using the [useGSAP](https://gsap.com/resources/React/#usegsap-hook-) hook along with the ScrollTrigger plugin, both of which need to be imported and registered outside the component.
...
<Aside title="GSAP Stagger Parameters">
  ...
  We define parameters for our staggered animation to control how elements animate progressively. Learn more about stagger options in the [GSAP documentation](https://greensock.com/docs/v3/Staggers).

  Key parameters include:

  - **each**: Delay between each element's animation (`0.05` seconds).
  - **from**: Animation starting point (`center` means the animation begins from the center of the grid).
  - **grid**: Automatically handles grid-based staggering (`auto` layout).
  - **ease**: Controls animation smoothness (`power2.inOut`).
...
Let's adjust our animations for mobile devices which would benefit from a different type of stagger because you only see 1 or 2 cards at a time.

By leveraging GSAP's `matchMedia` utility, we can conditionally run animations based on a media query.

Different Animations for Different Screens:

Mobile (max-width: 640px): Animations will kick off from the start of the grid, ensuring a smooth reveal on smaller screens.

Desktop (min-width: 641px): Animations start from the center of the grid for a more balanced, eye-catching effect.

We wrap our GSAP timeline logic in an animateFeatures function. This function accepts custom stagger parameters, so you can easily tweak how elements animate in based on the device.

Using matchMedia.add, we define two sets of stagger parameters: one for mobile and one for desktop. This way, GSAP applies the correct settings automatically.

```tsx title="AnimatedGrid.tsx" showLineNumbers
```


## Making it reusable: Passing the card data as Props

Let's finish things off by allowing our Grid Component to receive data via props instead.

We export the GridCardProps type and add a 'cards' prop to the component. We can then replace the hardcoded DATA array with the prop value.

```tsx {3,11} title="AnimatedGrid.tsx"
// ...
...
const AnimatedGrid: FC<Props> = ({ className, cards = [] }) => {
  // ...
  // Replace 'DATA' with 'cards' prop
}
```

### How does this help?
{/* These are the same thing obviously AI written needs to be more succinct */}
Server-Side Compatibility: This setup lets your server-side logic supply dynamic data to the client-side component, enhancing scalability and maintainability.

Dynamic Data Injection: The component now receives its card data via a prop (data), making it easy to pass dynamic content from a server-side parent.

Type Safety: Using CardGridProps ensures each card adheres to a consistent format—minimizing errors.

Reusability & Flexibility: With the data abstracted into a prop, this component is now a versatile building block that can be used across different parts of your application.
...
That's it! You've created a responsive, animated grid with **Next.js**, **GSAP**, and **Tailwind CSS**.

This reusable component is ready to enhance your site's interactivity.

Keep building and animating responsibly! :)