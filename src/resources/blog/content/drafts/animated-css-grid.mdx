...
We'll leave the desktop animation as it was - this timeline be added if the viewport is greater than 640px.

Narrow views (less than 640px) would benefit from a different type of transition because there is a single column, and you can only see a couple of cards at any one time. 

The grid stagger is ineffective because by the time the user sees the latter cards they will have already animated in.

Instead, let's make it so that on mobile the cards animate individually as they enter the viewport.
