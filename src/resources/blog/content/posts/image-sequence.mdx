...
Shoutout to [Derek Elliott](https://www.derrk.com/) for his awesome 3D work on the bottle images 🍾
...
- Large set of images needed for a smooth animation which can be heavy to load
- Limited interactivty compared to 3D models (no pointer interactions)
</Aside>

## Install Dependencies
To get started, ensure your Next.js project is setup with Tailwind CSS, and then install the following packages:
...
Once ready, stick them in your `public` directory so we can load them easily.
...
We need to generate an array of image sources based on the file naming pattern. In this case we have 60 frames.
...
- We set the loaded images into state
...
It returns a promise of image elements which we're setting into state.
...
- Or it will call `onImageError` if the image fails to load
...
**With this in place you have a responsive canvas that re-draws the current frame after resizing has ended!**
...
And remember: Animate responsibly 😉