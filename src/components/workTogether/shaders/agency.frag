...
float getSinTime () {
  return sin(u_time * 2.0) * 0.5 + 0.5;
}

float sdNoiseSphere(in vec3 p, in float s, in float index, in float t) {
  float amplitude = 0.06 * t;
  float n = noise(vec3(p.x * 2.0 - index, p.y * 2.0 - index, u_time * 1.2)) * amplitude;
...
void main() {
    // 1) Camera position around origin
    float angle = u_time * 1.5;
    vec3 ro = 1.5 * vec3(cos(angle), 0.0, sin(angle));

    // 2) Look-at direction points from ro → 0 => -ro
    vec3 forward = normalize(-ro);
    vec3 right = normalize(cross(forward, vec3(0.0, 1.0, 0.0)));
    vec3 up = cross(right, forward);

    // v_uv goes from -1..1 in both x and y (assuming you set it that way in the vertex shader).
    // We construct rd by adding a bit of “perspective” offsets in right and up:
    vec3 rd = normalize(forward + v_uv.x * right + v_uv.y * up);
    ...
    float t = getSinTime();
    vec3 sphereColour = mix(u_orange, u_light, t);
    sphereColour *= diffuse;

    // If distance == max distance, we didn't hit anything = color the background
    vec3 colour = mix(u_black, sphereColour, step(td, MAX_DISTANCE));
