...
void main() {
  ...
  float uvXVariation = fract(vUv.x * 2.0);
  ...
  for (int i = 0; i < NUM_LINES; i++) {
    ...
    // -------------------------------
    // Each line's parameters are stored consecutively:
    // index*3 gives xOffset, index*3+1 gives yExtension, index*3+2 gives thickness.
    ...
    float thickness  = uLinesParams[i * 4 + 2];      // Line thickness
    ...
    float posNoise = noise(vec3(vUv.y * 2.0 - uTime, vUv.x / 2.0, iF + xOffset)) * 0.5 - 0.5;
    // Compute a sine-based oscillation for further variation.
    // float posSin = sin(vUv.y * 4.0 - (iF * 0.1) + uTime) * 0.5 + 0.5;
    // Combine base UV, offset, noise, and sine contributions.
    float shiftedUV = uvXVariation + xOffset + posNoise * 0.12;
    // Multiply by horizontal variation computed earlier.
    // shiftedUV *= uvXVariation;
    
    // Create a horizontal mask: use smoothstep for a soft edge based on the thickness.
    // The factor "abs(shiftedUV - 0.5) * 2.0" centers the effect.
    float horizAlpha = 1.0 - smoothstep(thickness, thickness + 0.1, abs(shiftedUV - 0.5) * 2.0);
  ...
  finalColour.a *= 1.0 - smoothstep(0.5, 1.0, abs(vUv.y - 0.5) * 2.0);
