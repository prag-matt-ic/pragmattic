...
import { RotatedPlaneCanvas, BasicFragmentShaderPlaneCanvas, GradientFragmentShaderPlaneCanvas, GridLinesFragmentShaderPlaneCanvas } from './WavePlaneBlog'

<section className='w-full relative h-[620px]'>
  <WavePlane  />
</section>

## Pretty cool, right? 

This project is a great way to learn about <b>custom ThreeJS shaders</b> and how to use them in a <b>Next.js Typescript app</b>.<br/>
...
## Building The WavePlane Component

### Rotated Plane with Dynamic Size

The Wave Plane component is essentially just a plane geometry rotated 90 degrees on the x-axis to form a floor surface.

The width and height are set dynamically based on the viewport size. The viewport size is obtained via the <code>useThree</code> hook - which can only be called from inside the <code>Canvas</code> component.

We need to ensure there are enough segments for the vertices to displace smoothly. So the number of segments is a multiple of the plane size. If we were to leave this value at 1, we wouldn't see any distortion. Increasing the number of plane segments gives us a smoother wave effect at the cost of performance.
...
const WavePlane: FC<Props> = () => {
  ...
  return (
    <mesh position={[0, -viewport.height / 2.5, -1]} rotation={[-0.5 * Math.PI, 0, 0]}>
      ...
      <meshBasicMaterial color="blue" /> // We'll be adding our custom shader material here
    </mesh>
  )
}
...
As the name suggests, they are the same for each vertex or fragment in the shader.
It's good practice to define your uniforms as a type to mitigate errors.

#### Vertex Shader
The vertex shader runs simaltaneously for each point on the plane.
The vertex shader is responsible for transforming the vertices of the geometry. We'll use it to distort our plane to create the wave effect.

#### Fragment Shader
The fragment shader runs in parallel for each pixel on the plane.
The fragment shader is responsible for determining the colour of the pixel. This will allow us to shade the plane based on the height of the wave.
...
The following is an annotated version of a basic vertex shader for ThreeJS.
...
```
<i>Note: If you are using a ScreenQuad instead of a Plane - like some of my other examples - there's a simpler shader which reduces the number of transformations needed.</i>
...
Noise functions generate random values based on a given input. They are invaluable for creating organic looking effects such as clouds and terrain.
...
<b>Vertex Shader</b> <code>wavePlane.vert</code>
```glsl
...
// Varyings for passing information to the fragment shader
varying vec2 vUv;
...
- We create a noise input vec3 (noiseInput) using the X and Y positions of the vertex and the time uniform (uTime). We dividing the X and Y positions to make the waves smaller. (Try changing that value!)
- By adding uTime as the 3rd input vector we get a value that continously changes as the elapsed time increases
...
<b>Fragment Shader</b> <code>wavePlane.frag</code>
```glsl
void main() { 
...
We now have our vertex shader written and a basic fragment shader to test that it's working.
...
float brightenValue(in float value) {
   return clamp(value + 0.25, 0.0, 1.0);
}
...
In order to utilise GSAP ScrollTrigger, we need to register the plugins at the top of the file:
...
Let's create a ref for storing the scrollProgress, and set it inside the [useGSAP hook](https://gsap.com/resources/react-basics/) via a ScrollTrigger.

If you revisit the [live demo](/examples/wave-plane/demo) you can scroll til your heart's content. Literally, forever! That's because I've also added an infinite scroll loop inside the ScrollTrigger.
...
#### Summary
- We've setup our ScrollTrigger to start at 0 (top of the page) and end at the max scroll position (bottom of the page)
- We update the scrollProgress value inside the onUpdate function. The progress ranges from 0 to 1.
- For infinite scrolling. When the progress = 1, we increment scrollLoop, reset scrollProgress to 0, and scroll to the top of the window.
- We update the shader's uScrollProgress uniform inside useFrame. By adding the scrollLoop count we make a smooth transiton between the bottom and the top of the page after we loop.

Now that we have a live scroll progress value, we can incorporate it into our shaders to add an extra level of interaction.


### Incorporating scroll progress into the shaders

We can use the scroll progress uniform (uScrollProgress) to influence the noise and the line positions.
This creates the impression of the plane moving towards us as we scroll.
...
````glsl
    ...
    float linePosY = fract(vUv.y * uGridSize + yOffset); // <- Move the horizontal lines by adding the scroll offset
````
...
### [Final Result](/examples/wave-plane/demo)

You can find the full code for this project in my public [Github repo here](https://github.com/prag-matt-ic/pragmattic/tree/main/src/components/examples/wavePlane).
...
If you've coded along (and I've made things clear enough!) you should now have your own wave plane with scroll interaction.


#### Credits

I was originally inspired by [this blog post](https://blog.maximeheckel.com/posts/vaporwave-3d-scene-with-threejs/) by Maxime Heckel.

Thanks to my team for support and feedback.