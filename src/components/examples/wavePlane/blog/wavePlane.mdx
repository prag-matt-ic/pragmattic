...
<section className='w-full relative h-[704px]'>
  <WavePlane  />
</section>
...
<i>At the time of writing Next v15 (React 19) and React Three Fiber (v8) don't play well together - so you may need to use Next v14.</i>
...
```tsx title="WavePlaneCanvas.tsx" showLineNumbers
...
The <code>WavePlane</code> component is essentially a plane geometry rotated 90 degrees on the x-axis to form a floor surface.
...
```glsl title="Vertex Shader wavePlane.vert" showLineNumbers
...
- We create a noise input vec3 (noiseInput) using the X and Y positions of the vertex and the time uniform (uTime). We divide the X and Y positions to make the waves smaller. (Try changing that value!)
...
```glsl title="Fragment Shader wavePlane.frag" showLineNumbers
uniform float uTime;
...
```glsl {19-21,28-42} title="Fragment Shader wavePlane.frag" showLineNumbers 
...
### [Final Result ↗](/examples/wave-plane)

You can find the [final code here](https://github.com/prag-matt-ic/pragmattic/tree/main/src/components/examples/wavePlane).
...
I've found learning about shaders to be an extremely rewarding experience. They unlock so many creative possibilities.
At my startup Timeboxx I've written a background shader, and it really elevates the design in ways not otherwise possible.

I hope that you've found this article useful, and that it's inspired you to dive deeper in your own personal journey.
